...
class WSConnection extends platform_1.Duplex {
    ...
    connect(opts) {
        ...
        if (typeof this.socket.on === 'function') {
            this.socket.on('upgrade', (response) => {
                this.client.emit('hjon:upgrade', response.headers);
            });
        }
        this.socket.onopen = () => {
            this.emit('connect');
    ...
    handleSocketError(error) {
        this.client.emit('hjon:socketerror', error);
        if (error.code === 'ECONNRESET') {
            this.client.emit('hjon:socketerror:reset', error);
        }
    }
